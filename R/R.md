...
함수에 따라 **1) 연산 대상 데이터의 종류, 2) 결과 출력 형태, 3) 연산 단위**가 달라짐.

```R
apply(iris_num, 1, mean, na.rm = T)  #1->행, 2->열, na.rm=T->na제거하고 연산
...
lapply
input ==>데이터프레임(data frame)인 경우 length( 데이터 )의 결과는 변수의 개수(열의 개수)이고, 리스트인 경우 length( 데이터 )는 리스트 인자의 개수 

output ==> list, 리스트의 인자는 length( 데이터 ) 만큼 생성. 

```R
aggregate(data = teens, age~gradyear,mean,na.rm=TRUE) #gradeyear로 그룹을 나누어서 age 평균을 구함. 데이터 프레임으로 출력 .
ave(teens$age, teens$gradyear,FUN=function(x)mean(x,na.rm=TRUE)) #졸업 년도에 따라 age의 평균을 구하겠다. 벡터로 출력 
```



- 정규화(normalization, rescaling) : (각값 - 최소값) / (최대값- 최소값)  #수에 대한 특성을 따지지 않고 0에서 1사이로 만듦. 
- 표준화() : (각값 - 평균) / 표준 편차  #음수면 값이 평균보다 작고, 양수면 평균보다 큰 값이다. 표준 편차로 나눔으로써 정규분포 형태의 그래프로 출력 , 일반적으로는 표준화를 더 많이 사용. 토익/ 토플점수 =>   
