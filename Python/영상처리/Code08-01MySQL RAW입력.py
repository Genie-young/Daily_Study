...
DB_NAME = 'BigDataDB'; CHAR_SET = 'utf8'
####################
#### 함수부 ####
####################
def selectFile() :
...
def uploadData() :
    ...
    upUser = USER_NAME
    sql = "INSERT INTO rawImageTBL(raw_id , raw_height , raw_width"
...
def downloadData() :
    con = pymysql.connect(host=IP_ADDR, user=USER_NAME, password=USER_PASS,
                          db=DB_NAME, charset=CHAR_SET)
    cur = con.cursor()
    sql = "select raw_fname, raw_data from rawImageTBL where raw_id =1"
...
btnUpload2 = Button(window, text="다운로드", command=downloadData); btnUpload2.pack()
